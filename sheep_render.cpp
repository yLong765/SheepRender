...
int main() {
    screen screen = screen::create(width, height, "sheep render");
    ...
    while (GetMessage(&msg, screen.handle, 0, 0)) {
        ...
        for (int i = 0; i < model.mesh.triangles.size(); i += 3) {
            vec4 p1 = vec4(model.mesh.vertices[i].point, 1);
            vec4 p2 = vec4(model.mesh.vertices[i + 1].point, 1);
            vec4 p3 = vec4(model.mesh.vertices[i + 2].point, 1);
            p1 = p1 * trans;
            p2 = p2 * trans;
            p3 = p3 * trans;
        ...
        //renderer.draw_line(0, 0, 800, 600, color(1.0f, 0.0f, 0.0f));
